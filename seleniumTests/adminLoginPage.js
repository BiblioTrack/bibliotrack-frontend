...
function logResults(result) {
    ...
    fs.appendFile('adminLoginLog.txt', logEntry, (err) => {
        if (err) throw err;
    });
}
...
async function adminLoginPage() {
    ...
    try {
      ...
      logResults(`admin logged out successfully and URL is ${postLogoutUrl}`);
      await driver.sleep(2000);
      logResults('admin login and log out functionality tested');
